...
# ----------------------------------------------------------------
@st.cache_resource
def load_models_and_features(version):
    models = {}
    version_dir = os.path.join(MODEL_DIR, version)
    for h in [5, 15, 30, 60]:
        model_path = os.path.join(version_dir, f"lgb_model_{h}m.pkl")
...
def compute_features_at_time(df_history, feature_cols):
    ...
    compute the identical 132 features expected by the LightGBM models.
    """
    # Grab the last 61 rows to compute lags (up to 60) and rolling windows
...
selected_version = st.sidebar.selectbox(
    "Model Version",
    ["v2_tuned", "v1_baseline"],
    index=0,
    help="v1_baseline: Original models. v2_tuned: Optuna hyperparameter-tuned models with higher precision."
)

models, feature_cols = load_models_and_features(selected_version)
...
# Dashboard Layout
# ----------------------------------------------------------------
st.title("🔥 Honeywell OPC UA Time-Series Alarm Predictor")
st.markdown("### Simulated Real-Time Control Panel")
...
if pred_5m >= ALARM_LIMIT:
    ...
    alert_type = "Warning"
    rec = "Tactical Warning: Temperature predicted to cross 21.0 in 30 minutes. Action: Verify that the 3E104 Level control (03LIC_1016.PV) is stable. Check pump status."
elif pred_60m >= ALARM_LIMIT:
    alert_type = "Advisory"
    rec = "Strategic Advisory: High temperature forecast in 60 minutes. Action: Monitor inlet feed flow rate (02FI_1000.PV) and prepare to adjust suction pressure."
else:
    alert_type = "Normal"
    rec = None
...
with c_scen:
    ...
    st.markdown(f"**Model Version:** `{selected_version.upper()}`")
...
if st.session_state.is_playing:
    # Speed calculation: sleep time decreases as speed increases
