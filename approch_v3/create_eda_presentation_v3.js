...
// ============================================================================
// Slide 16B: 3-Phase Feature Selection Pipeline & SHAP Importance (New Slide)
// ============================================================================
const s16bTable = [
  ["Feature Category", "Pruned Features (15m)", "SHAP Mean Abs Value"],
  ["03PIC_1023.PV", "Separator Pressure (Raw)", "0.5841"],
  ["03TIC_1023.PV_lag_30", "Overhead Temp (Lag 30m)", "0.2415"],
  ["03TI_1024.PV_roll_min_60", "Bottom Temp (Roll Min 60m)", "0.1982"],
  ["hour", "Hour of Day (Time Context)", "0.1107"],
  ["03TIC_1023.PV_diff_30", "Overhead Temp (Diff 30m)", "0.0894"]
];
createSlideWithAssets(
  "3-Phase Feature Selection (Limited Features)",
  "Pruning 113 Features down to 25",
  [
    { text: "Phase 1: Collinearity Filter (|r| > 0.90):\n", options: { bold: true, color: COLOR_NAVY } },
    { text: "Pearson correlation drop to prevent credit-splitting. Kept lower complexity calculation structures (Raw > Lag > Diff > Roll).\n\n", options: { color: COLOR_SLATE } },
    { text: "Phase 2: Permutation Importance Filter:\n", options: { bold: true, color: COLOR_NAVY } },
    { text: "5-fold TimeSeriesSplit. Features with zero or negative validation MAE importance (e.g. calendar year/month drift) are dropped. Retained top 25.\n\n", options: { color: COLOR_SLATE } },
    { text: "Phase 3: SHAP TreeExplainer Filtration:\n", options: { bold: true, color: COLOR_NAVY } },
    { text: "Filtered out features with mean absolute SHAP value < 10^-4. Verified thermodynamic directionality (e.g. Bottom Temp drives Overhead Temp up).", options: { color: COLOR_SLATE } }
  ],
...
// ============================================================================
// Slide 16C: Approach v3 - All Engineered Features (New Slide)
// ============================================================================
const s16cTable = [
  ["Feature Category", "Count", "Reasoning / Status"],
  ["Lags", "35", "Retained - Momentum & History"],
  ["Rolling Stats", "60", "Retained - Baselines & Stability"],
  ["Differences", "15", "Retained - Heating/Cooling Speed"],
  ["Time Context", "3", "Retained - Ambient Diurnal Cycles"],
  ["Raw Sensors", "19", "Retained - Base Operating States"]
];
createSlideWithAssets(
  "Approach v3: All Engineered Features",
  "Bypassing Pruning for Process Complexity",
  [
    { text: "Bypassed Feature Selection:\n", options: { bold: true, color: COLOR_NAVY } },
    { text: "Instead of choosing 25 features per horizon, the models in Approach v3 are trained directly on all 132 raw and engineered features.\n\n", options: { color: COLOR_SLATE } },
    { text: "LightGBM Split Capacity:\n", options: { bold: true, color: COLOR_NAVY } },
    { text: "Tree ensembles like LightGBM choose splits greedily. It handles multi-collinear associations natively without premature information loss.\n\n", options: { color: COLOR_SLATE } },
    { text: "Comparison Objective:\n", options: { bold: true, color: COLOR_NAVY } },
    { text: "By keeping the full library, we compare the parsimony of the 25-feature pruned model against the raw thermodynamic power of all 132 features.", options: { color: COLOR_SLATE } }
  ],
  "pearson_correlation_heatmap.png",
  s16cTable,
  [1.5, 0.8, 2.3]
...
// ============================================================================
// Slide 19B: Model Performance Comparison (New Slide)
// ============================================================================
const rawComp = readCSV("comparison_selected_vs_all_features_summary.csv");
let s19bTable = [
  ["Horizon", "F1 Selected", "F1 All", "Diff F1", "FAR Selected", "FAR All", "Diff FAR"]
];
if (rawComp.length > 1) {
...
createSlideWithAssets(
  "Model Comparison: Selected vs. All",
  "Tuned Models Performance Analysis",
  [
    { text: "Classification & Alarm Gains:\n", options: { bold: true, color: COLOR_NAVY } },
    { text: "All Features (132) outperforms Selected Features (25) at early horizons. F1-Score improves by +2.73% at 5m and +2.90% at 30m.\n\n", options: { color: COLOR_SLATE } },
    { text: "False Alarm Rate (FAR) Halved:\n", options: { bold: true, color: COLOR_NAVY } },
    { text: "At 5m, FAR drops from 0.2003% to 0.1080% (a 46% reduction). Similar reductions are observed at 15m and 30m, which minimizes operator alarm fatigue.\n\n", options: { color: COLOR_SLATE } },
    { text: "Training & Parsimony Trade-off:\n", options: { bold: true, color: COLOR_NAVY } },
    { text: "Selected Features trains in under 30s. All Features tuned training takes 73 mins for 5m. Selection provides huge computational savings.", options: { color: COLOR_SLATE } }
  ],
  "monthly_seasonal_trends.png",
  s19bTable,
  [0.8, 0.7, 0.7, 0.5, 0.7, 0.7, 0.5]
);
...
// Compile file
const outputPath = path.join(__dirname, "..", "docs", "EDA_Presentation_v3.pptx");
