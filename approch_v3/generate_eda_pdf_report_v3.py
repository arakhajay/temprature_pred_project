...
def generate_pdf():
    pdf_path = os.path.join("docs", "EDA_Detailed_Report_v3.pdf")
    ...
    # ============================================================================
    # Section 9: Model Performance Comparison (Selected vs. All Features)
    # ============================================================================
    story.append(Paragraph("9. Model Performance Comparison: Selected Features vs. All Features", h1_style))
    story.append(Paragraph(
        "To evaluate the trade-offs between model parsimony and model complexity, we implemented "
        "<b>Approach v3</b>. This approach completely bypasses the 3-phase feature selection pipeline and "
        "trains the LightGBM models directly on all 132 engineered and raw process features (raw variables, "
        "lags, rolling averages, differences, and time features). The table below compares the performance of "
        "the tuned models trained on 25 selected features against the tuned models trained on all 132 features.",
        body_style
    ))
    
    t_comp_v3 = build_report_table("comparison_selected_vs_all_features_summary.csv", max_rows=5, custom_col_widths=[75, 75, 75, 75, 75, 75, 75])
    if t_comp_v3: story.append(t_comp_v3)
    story.append(Spacer(1, 10))
    
    story.append(Paragraph("Comparative Performance Insights:", h2_style))
    story.append(Paragraph("&bull; <b>Precision and F1-Score Gains:</b> Bypassing the feature selection pipeline improves F1-score at early horizons (e.g. +2.73% at 5 min; +2.90% at 30 min) by allowing LightGBM to greedily discover non-linear combinations from all process feedback variables.", bullet_style))
    story.append(Paragraph("&bull; <b>Reduction in False Alarm Rate (FAR):</b> The False Alarm Rate is cut in half at 5 minutes, dropping from 0.2003% to 0.1080%. This represents a major benefit for control room operator confidence.", bullet_style))
    story.append(Paragraph("&bull; <b>Parsimony Trade-off:</b> Training on all 132 features significantly increases model complexity and training time (Optuna hyperparameter tuning took 73 minutes for 5 min, compared to under 30 seconds for the Selected Features subset). Using 25 selected features remains a highly efficient, thermodynamically explainable alternative.", bullet_style))
    ...
    # ============================================================================
    story.append(Paragraph("10. Actionable Recommendations", h1_style))
    story.append(Paragraph(
